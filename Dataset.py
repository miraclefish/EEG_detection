...
class Dataset:
    ...
    def getData(self, shuffle=False, fft_process=False, L=None, norm=False, only_P=True):
        data_X, data_Y = self._preProcess(fft_process=fft_process, L=L, norm=norm, only_P=only_P)
        if shuffle:
    ...
    def _preProcess(self, fft_process=False, L=None, norm=False, only_P=True):
    ...
    def plot_EEG_data(self, mode="demo", data=None, ratio=1.0, placement=120):
...
# dataset = Dataset("myoclonus")
# dataX, dataY = dataset.getData(norm=False, fft_process=False, only_P=True)
# for i in range(dataX.shape[0]):
#     data = np.concatenate([dataX[i], dataY[i][:,np.newaxis]], axis=1)
#     dataset.plot_EEG_data(mode="test_show", data=data)
#     # plt.savefig("image-show\\"+str(i)+".png")
# pass