...
def test(dataset_name, epoch):
    ...
    while i<len_dataloader:
        ...
        output = net(x_data=x_data, x_feature=x_feature)

        if i == len_dataloader-1:
            label_total[i*batch_size:] = label.data.numpy().squeeze()
            output_total[i*batch_size:] = output.data.numpy().squeeze()
        else:
            label_total[i*batch_size:(i+1)*batch_size] = label.data.numpy().squeeze()
            output_total[i*batch_size:(i+1)*batch_size] = output.data.numpy().squeeze()
