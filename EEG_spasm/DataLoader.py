...
class Dataloader:
    def __init__(self, batchsize, fft_process=False, norm=False, L=None):
    ...
    def _load_data(self, shuffle=True):
        ...
        data_X, data_Y = dataset.getData(fft_process=self.fft_process, norm=self.norm, only_P=self.only_P, L=self.L)
        if shuffle:
...
# dataload = Dataloader(batchsize=32, fft_process=False, norm=True, L=138)
# pass